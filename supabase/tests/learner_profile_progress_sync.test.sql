...
create function pg_temp.resize_learner_profile_envelope(
  p_envelope jsonb,
  ...
  for attempt in 1..8 loop
    candidate := pg_catalog.jsonb_set(
      ...
      pg_catalog.jsonb_build_array(pg_catalog.jsonb_build_object(
        ...
        'id', 'boundary-channel',
        'imageUrl', '',
        'name', pg_catalog.repeat('x', filler_length)
      ))
    );
    candidate := pg_temp.rehash_learner_profile_envelope(candidate);
    measured_bytes := (
      candidate #>> '{integrity,byteLength}'
    )::integer;
    exit when measured_bytes = p_target_bytes;
    filler_length := filler_length + p_target_bytes - measured_bytes;
    if filler_length < 1 then
      raise exception 'Target learner profile is too small';
    end if;
  end loop;
  if measured_bytes <> p_target_bytes then
    raise exception 'Target learner profile byte length did not settle';
  end if;
  return candidate;
end;
$$;
...
create temporary table boundary_learner_profile_envelopes (
  case_name text primary key,
  envelope jsonb not null
) on commit drop;

insert into boundary_learner_profile_envelopes (case_name, envelope)
select
  'unsupported',
  pg_temp.rehash_learner_profile_envelope(
...
select throws_ok(
  $query$
    ...
    from public.commit_my_learner_profile(
      ...
      (
        select envelope
        from boundary_learner_profile_envelopes
        where case_name = 'unsupported'
      )
    )
  $query$,
  '22023',
  'Learner profile envelope is invalid',
  'an unsupported portable schema version is rejected'
);

select results_eq(
  $query$
    select status, revision, payload_sha256
    from public.commit_my_learner_profile(
      ...
      (
        select envelope
        from boundary_learner_profile_envelopes
        where case_name = 'exact-boundary'
      )
    )
  $query$,
  $expected$
    ...
    where case_name = 'exact-boundary'
  $expected$,
  'the server accepts an exact 2097152-byte canonical envelope'
);
  ...
  $query$
    ...
    from public.commit_my_learner_profile(
      ...
      (
        select envelope
        from boundary_learner_profile_envelopes
        where case_name = 'oversize'
      )
    )
  $query$,
  '22023',
  'Learner profile envelope is invalid',
  'the server rejects a canonical envelope one byte above the cloud bound'
);
  ...
  $query$
    select
      ...
      (
        select count(*)
        from public.learner_profile_versions as version
        where version.user_id = head.user_id
      ),
      (
        select count(*)
        from public.learner_profile_write_receipts as receipt
        where receipt.user_id = head.user_id
      ),
      unrelated.revision,
      (
        select count(*)
        from public.learner_profile_versions as version
        where version.user_id = unrelated.user_id
      )
    from public.learner_profile_heads as head
    cross join public.learner_profile_heads as unrelated
    where head.user_id = '11111111-1111-4111-8111-111111111111'
      and unrelated.user_id = '22222222-2222-4222-8222-222222222222'
  $query$,
  $$values (3::bigint, 3::bigint, 2::bigint, 1::bigint, 1::bigint)$$,
  'rejections preserve accepted versions, receipts, heads, and another owner'
);
