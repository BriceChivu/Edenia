...
When sandbox mode is opened with no saved sandbox state, the app creates demo progress automatically. The header shows a `Sandbox` badge plus `Add day`, `Demo`, and `Reset` actions. `Add day` appends a random sandbox-only study day after the latest sandbox activity so future heatmap and city-progress states can be explored quickly. Sandbox demo data includes watched videos, Anki logs, unlocked city images, and future-dated activity for heatmap previews. It does not touch the normal `studybuild_v1` progress state.
