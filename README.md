...
Production analytics begin with a browser identity. After successful authentication, Edenia identifies that existing PostHog person with the stable Supabase UUID so the pre-authentication journey stays attached. The normalized account email is the only account person property sent for identification and support. Edenia does not use or send the Auth method, provider subject, provider labels, or name as analytics identity authority, and it does not copy the account email into ordinary event payloads. Sign-out resets the browser identity; changing accounts resets it before identifying another UUID.

Autocapture is disabled, but session recording remains enabled for ordinary product behavior. Replay masks every input value and blocks Auth surfaces. A page whose live URL contains an authentication secret key starts with replay paused and resumes only after the callback URL is safe. Captured URLs omit fragments and replace authentication secret values in query parameters with `[REDACTED]`; ordinary campaign and product URL parameters remain available. Edenia records controlled button actions, raw trimmed search queries, channel additions and removals with channel IDs and names, individual video additions and whether their cards became visible after the reveal attempt, onboarding and starter-feed results, aggregate daily study progress, streak changes, current and earned town levels, current settings, YouTube refresh results, successful Anki refreshes with their timestamps and summary counts, video opens, playback-session summaries, Favorite and video-placement changes, per-channel video-format views, Study Guidance impressions and actions, and watched-state changes. Each person profile includes the current watched-video IDs and count plus the current removed-video count; watched and unwatched events include the video ID, title, channel ID, watched timestamp, duration, source, and short-video status. Existing local study days, configured channels, and watched videos are synchronized once, then only changed values generate additional state events.

Submitting the feedback form sends its category, message, optional name and email, page and display context, and the current session-replay URL to PostHog. That optional feedback email is sent only when a learner enters it in the feedback form; Edenia does not fill it from the authenticated account. Custom analytics events do not include YouTube API keys, sync-file contents, the full serialized browser state, migration ciphertext, or a migration capability. PostHog is not initialized on localhost, alternate domains, sandbox mode, other paths, or a page load that has just received a legacy-progress outcome fragment.
