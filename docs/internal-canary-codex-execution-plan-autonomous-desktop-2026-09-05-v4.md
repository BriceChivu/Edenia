...
- **Specification status:** Approved by the product owner on 2026-09-05; tracker preparation completed and verified. The owner-approved v4 device amendment supersedes v3. Packet 0 (#290) was invoked and paused during v3 preflight; its device-only pause is superseded. Execution and remaining acceptance are incomplete.
...
- **Execution boundary:** This specification does not start work by itself. After this exact Plan ID is approved, one invocation naming a ticket—or saying `Execute the next ticket under internal-canary-codex-autonomous-2026-09-05-v4`—authorizes the complete bounded ticket workflow defined below for one selected top-level ticket, including the bounded derived repairs defined below. Codex must not ask again for an operation already included in that workflow.
...
The owner approved v3 and its tracker preparation on 2026-09-05. The [historical preparation receipt](https://github.com/BriceChivu/Edenia/issues/196#issuecomment-5552413464) records that original specification, issue mapping, and dependencies. The later explicit device-coverage amendment below adopts v4. The issue mapping is unchanged; this amendment updates existing managed contracts rather than creating duplicate tickets.
...
All nine issues remain open. #290 is assigned and its existing invocation carries into v4; other top-level tickets have not been invoked. Preserve all ten #196 acceptance checkbox states and audit IDs; update only the explicitly amended criterion texts 196-AC-02 and 196-AC-05.

## Approved v4 device-coverage amendment

The product owner approved this amendment on 2026-09-05: “yes, revise the plan. But don't even keep real-iPhone and paired-device acceptance as mandatory blockers before final release readiness”. This adopts Plan ID `internal-canary-codex-autonomous-2026-09-05-v4` and authorizes the corresponding tracker contract changes. It supersedes v3's device requirements and its device-only preflight pause; the remaining authority and safety boundaries continue unchanged.

- Actual current macOS Chrome, macOS Safari, a fresh isolated Chrome context on the same Mac, and applicable private-browsing coverage remain required.
- Real-iPhone Safari and a separate physical device are optional supplemental coverage. Their absence is never an execution prerequisite, an Exceptional pause, a missing required report target, a closure blocker for Packet 0 or #196, or a prerequisite for a later product-owner cutover decision. No purchase, device account, device connection, or manual phone acceptance is required.
- Synchronization, conflict choice, reload, sign-out, Start over, Undo, and preservation remain required. Exercise their multi-context behavior in independently persisted browser contexts on the same Mac. Separate tabs sharing the same browser storage are insufficient. Same-Mac coverage must be labeled accurately and never described as proof of separate hardware or actual iPhone Safari.
- Record unperformed optional device coverage as a non-blocking confidence gap, without a PASS assertion. An observed ownership or progress-loss defect still blocks acceptance regardless of the optional platform where it was discovered.
- This explicit amendment permits changing only #196 criterion texts `196-AC-02` and `196-AC-05` outside its managed block; preserve their checkbox states and the other eight criteria. Preserve v3 receipts as historical evidence, mark the former device-only pause superseded, and carry the existing #290 invocation into v4. Do not select a second top-level ticket. A later invocation naming historical v3 resolves to this adopted v4 tracker contract; it must not reactivate the superseded device requirements.
- Packet 0 must align the executable readiness validator, its contract tests, and the runbook with this coverage policy before declaring its support delivered. Until that reviewed change is delivered, the old validator is known to enforce superseded device targets; do not fabricate records or claim it already implements v4.

## Revision boundary

v4 changes device coverage and the two corresponding #196 criterion texts only. It preserves the product contracts, local validation matrices, provider rule, hosted desktop verification, recovery, review, delivery, monitoring, and explicit closure requirements from v3. Earlier documents and receipts remain historical. The plan and tracker amendment is authorized now; Packet 0's remaining implementation must still complete under its existing invocation and delivery contract. It is not completed by revising this specification.
...
```text
Approve plan internal-canary-codex-autonomous-2026-09-05-v4 and prepare its tracker items.

After reviewing each completed ticket:
Execute the next ticket under internal-canary-codex-autonomous-2026-09-05-v4.
```
...
```text
Execution contract: internal-canary-codex-autonomous-2026-09-05-v4
...
Every newly created issue begins with the **Mandatory issue execution capsule** and encloses all plan-owned content between `<!-- codex-plan:internal-canary-codex-autonomous-2026-09-05-v4:start -->` and `<!-- codex-plan:internal-canary-codex-autonomous-2026-09-05-v4:end -->`. For existing #286 and #196, preserve the pre-existing body except the explicitly authorized #196 AC-02 and AC-05 text amendments. Replace the existing v3 managed marker pair with one v4 block; never append conflicting active contracts. The original #196 checkbox state may change only during its final criterion audit as specified below. A later update may replace only content between those exact markers.
...
4. Add one coordination comment to #196 containing the Plan ID, sanitized historical baseline, created issue links, and blocker rationale. Append or replace only #196’s managed execution block; preserve the v4-effective body and leave its acceptance checkbox states unchanged during preparation. Add the execution prerequisites Packet 0 -> each remaining plan ticket and Packet 6 -> #286 as native blocked-by relationships, plus the #196 release blockers justified below. Store the full ordered packet-to-issue mapping in #196’s execution block so a fresh task can resolve “next ticket” from the tracker alone.
...
| --- | --- | --- |
| Packet 0 | Always use `Refs #<n>` | Close after reviewed support delivery, all capability checks and local recovery/resumption rehearsals pass, and the complete executable scenario manifest is durably recorded; unavailable required desktop-browser or operator access leaves it open; optional iPhone or separate-device access never blocks closure |
...
Acceptance audit: one PASS/FAIL line per applicable packet and v4-effective issue criterion with an evidence pointer; conditional criteria may be NOT APPLICABLE only with the exact terminal-branch rule:
...
### Required readiness-tool alignment

Packet 0 owns the narrow implementation in `scripts/release-readiness.mjs`, `tests/contracts/release-readiness.test.mjs`, and `docs/release-readiness-canaries.md` needed to make the v4 policy executable. Verify their current remote-base equivalents before editing.

- Separate required desktop targets (`macos-chrome`, `macos-safari`, `fresh-chrome-isolated-context`, and `private-browsing` under its existing applicability rule) from optional supplemental `ios-safari` and `fresh-chrome-paired-device` targets. Required-target completeness must never demand either optional target. Keep optional records truthful and validate their provenance if supplied.
- A missing optional record must not create a missing-target error, a skipped-required-case error, a blocked readiness status, or a failed closure audit. Preserve all required scenario, provider, deployed-schema, deployment identity, monitoring, ownership, and progress-preservation predicates.
- Replace physical-device requirements for synchronization and related multi-context cases with two independently persisted browser contexts on the same Mac. Keep separate session/storage state, explicit operation counts, owner-bound fixtures, and protected-history assertions. Do not replace live desktop or provider acceptance with mocks.
- Add focused behavioral contracts proving a complete desktop-only report can reach the existing `awaiting-product-owner-approval` state without either optional target, missing required desktop targets still block, and unsafe or invalid required evidence still fails. Verify that a confidence-gap statement about optional device coverage does not become a blocker or a false passing record.
- Update report examples, fixtures, and runbook wording; preserve old report provenance rather than relabeling historical device evidence. Run the focused readiness contracts and full shared delivery checks, obtain exact-SHA independent review, and deliver this tooling change as part of Packet 0 before its closure. This plan-amendment operation alone does not implement or deploy those source changes.
...
- [ ] Actual current macOS Chrome, macOS Safari, a fresh isolated Chrome context on the same Mac, and applicable private-browsing mode are accessible to Codex. Verify actual browser/OS identity and a harmless local or neutral-page interaction. Real-iPhone Safari and a separate physical device are optional, non-blocking supplemental coverage; missing access does not pause this ticket.
...
- [ ] The readiness validator, focused contracts, fixtures, and runbook implement the required desktop/optional device policy above; a valid desktop-only report needs no optional-device record.
- [ ] Each manifest row has executable steps and measurable assertions; every required subflow maps to a v4-effective #196 criterion. The manifest has no unresolved required desktop-browser, runner, evidence-access, or recovery placeholder. Unavailable optional iPhone or separate-device coverage is explicitly non-blocking.
...
3. Add one coordination comment to #196 with stable evidence links or embedded sanitized evidence, every issue link, and the exact dependency rules below. Preserve its v4-effective body and acceptance checkbox states during preparation, and append the complete #196 managed execution contract required above.
...
Tracker preparation is a one-time setup operation under specification approval, not a partial execution of any packet. After preparation, `Execute the next ticket under internal-canary-codex-autonomous-2026-09-05-v4` selects the first open eligible item below. If an item is already closed, Codex verifies its canonical receipt and skips it. A request naming a later ticket may select it only when its prerequisites are already satisfied.
...
| `browser-matrix` | Operate actual current macOS Chrome, macOS Safari, a fresh isolated Chrome context on the same Mac, and applicable private mode; record versions and concrete UI assertions. Real-iPhone Safari and separate physical-device checks are optional and non-blocking | Final `off`; live device/browser artifacts |
...
| `profile-sync-conflict` | Use two independently persisted browser contexts on the same Mac with the approved owner; prove local-first save, sequential propagation, competing accepted revisions and explicit conflict choice; compare protected study-fact invariants. A separate physical device is optional | `developer-canary`; isolated-browser-context artifacts |
...
Every injected clock, transport error, fixture, mocked component, and real provider interaction is labeled at subcase level. Local mocks cannot replace required live provider or desktop-browser evidence. Optional iPhone and separate-device coverage may be omitted with a non-blocking confidence-gap statement. Recovery preconditions and acceptable postconditions are specific to each scenario; rehearse them before any live mutation. If new support changes the release artifact or runtime, deliver and review it before starting a new soak.
...
### V4-effective #196 acceptance audit and closure

The explicit v4 amendment replaces only criterion texts 196-AC-02 and 196-AC-05; their checkbox states remain unchanged. Subsequent tracker preparation preserves the ten acceptance criteria as amended by v4 and assigns stable audit IDs `196-AC-01` through `196-AC-10` in the managed block. Record a hash of their text excluding checkbox state. Before closure, re-read the live issue and reconcile any changed criterion; do not overwrite concurrent edits.

Map each v4-effective criterion and every managed criterion to the exact scenario artifacts and reviewed result. The #196 invocation authorizes changing only those original ten acceptance checkbox states whose mapped evidence passes, plus the managed acceptance boxes. Preserve the v4-effective wording, ordering, and all unrelated text. If criteria have changed materially outside this plan, identify the exact scope conflict before continuing.

A passing report alone is insufficient. The separate reviewer reads the underlying sanitized runner artifacts, verifies all required desktop browsers and subflows actually ran, confirms injected versus live evidence is accurately labeled, verifies no required case was skipped, and audits the final gate/monitor cleanup. Conditional non-applicability requires a named runbook rule and rationale; missing required access is a failure to complete, not non-applicability. Optional iPhone and separate-device coverage is outside the mandatory matrix and never blocks completion.

Closure requires all ten v4-effective criteria and all applicable managed criteria to pass, all bounded blockers closed, a validated report requesting product-owner cutover approval, no known ownership/progress defect, verified gate `off` with no owner, disabled bounded canary, and the heartbeat restricted to closure reconciliation. Verified automation cancellation and lease release complete the receipt immediately after closure. Post the exact report/artifact identities and criterion mapping before closing, then verify the rendered issue and checkbox state. This closes internal evidence collection only; #197 remains a separate owner decision.
...
## Exact amended #196 acceptance texts

**196-AC-02** — Current macOS Chrome, current macOS Safari, a fresh isolated Chrome context on the same Mac, and an applicable private-browsing smoke are covered. Real-iPhone Safari and separate physical-device coverage are optional and do not block release readiness or issue closure.

**196-AC-05** — Local-first saving, sequential synchronization between two isolated browser contexts on the same Mac, competing revisions, conflict choice, failed backup, oversize or corrupt rejection, export, import, missing-head recovery, Start over, and Undo are demonstrated without progress loss. Separate physical-device acceptance is optional.
