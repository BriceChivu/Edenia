# Internal canary Codex execution plan — 2026-09-04

## Status and purpose

- **Status:** Execution-oriented draft for product-owner approval.
...
- **Basis:** [the corrected review](internal-canary-issue-plan-corrected-2026-09-04.md), [the exploratory record](internal-canary-exploratory-2026-09-04.md), current repository guidance, ADR-0001, the release-readiness runbook, current source, and current tracker state.
...
| --- | --- | --- | --- |
| `tracker-preparation` | Reconcile the exact labels below; create or update the named issues, comments, links, and dependencies using already published sources | An explicit request to perform tracker preparation | Every intended tracker item exists with the exact classification and assembled self-contained body defined below |
| `local-issue-work` | Inspect source/history; create focused regressions; edit local source and tests; run local validation | An explicit request to diagnose or implement the named issue | The local work is complete and reviewed; no remote or hosted state changed |
...
| `provider-execution` | Use the approved Google/Supabase operator surfaces to apply and inspect the chosen configuration; update repository documentation separately | A recorded product-owner choice plus an explicit request authorizing the exact provider surfaces | Configuration and rollback evidence are recorded; live account/browser checks still require `hosted-safe-smoke` authority |
...
If a task lacks the authority for its next stage, Codex stops at the current stage, records what is complete, and asks for only the missing authority. It does not infer push, PR, merge, deployment, provider, gate, or hosted-profile authority from an issue label or from this document.

## Tracker preparation

### 1. Publish stable execution sources

Before creating or updating the proposed tracker items:

1. Review this document and the sanitized exploratory record.
2. With separate `delivery` authority, commit and publish those documents, then record their stable repository commit URLs.
3. Use those URLs only as supporting context. Each issue body must still contain its own baseline, starting points, acceptance criteria, validation, and stop conditions.
4. Publish no credentials, private provider/project endpoints, emails, UUIDs, tokens, sessions, raw database rows, learner-profile contents, or local filesystem paths. The public Edenia route and official documentation URLs may remain.
...
Tracker preparation remains blocked until the stable URLs exist. Authority to create or update tracker items does not itself authorize committing or publishing the source documents.
...
For Packets 1 and 3–7, the GitHub issue body is the complete packet plus the **Shared Codex completion contract** below. Include the shared contract in the issue itself; a link to this plan is supporting context, not a substitute. Packet 2 instead includes the Authority contract, its full decision/execution packet, and its provider-specific preconditions. Do not create an issue from only the tracker-inventory row, proposed title, or summary.

## Shared Codex completion contract

Every source-changing packet below uses this process in addition to its packet-specific criteria:
...
Local completion never implies delivery or hosted verification. Delivery and hosted steps use the authority contract above.
...
After deployment, `hosted-safe-smoke` authority may allow only the approved internal-canary account to sign in, retry profile opening, reload, and sign out. It must not construct malformed metadata, import, restore, corrupt records, change study content, or perform Start over. Hosted success is claimed only against the deployed fix SHA and a verified safe terminal outcome.
...
- Codex owns the investigation, recommendation, provider operation, verification, and documentation.
...
- While waiting for that choice, replace `needs-triage` with `needs-info` only after Codex has posted its recommendation and exact rollback.
- Do not apply `ready-for-agent`: an AFK label is not authority to mutate Google or production Supabase Auth.
- After the explicit choice and execution request, remove the waiting label, assign the interactive task to Codex, and retain `bug` until verified.
...
- [ ] Repository documentation is updated in a separate source diff where the chosen contract changes it.
...
The `ready-for-agent` task is local implementation and validation only. Hosted interaction is deferred to a separately authorized #196 rerun; it must never select `Try backup again` merely to verify this UI fix.
...
Use the heatmap day's existing complete `aria-label` as the only screen-reader presentation. Keep `#heatmapTooltip` a visual-only element with `aria-hidden="true"` at all times. Do not add `role="tooltip"` or `aria-describedby` and do not duplicate the day details in two accessible presentations.
...
- [ ] The active day's existing `aria-label` remains the single complete accessible presentation.
...
- [ ] Fetch and record the current remote `master` SHA without discarding the #286 branch.
- [ ] Reproduce the same deterministic fixture on a clean worktree at current `origin/master` and on the #286 branch.
...
- **Branch-only:** Add the failing regression and repair to #286, remove this ticket as a #196 candidate blocker, and close it as folded into #286.
- **Current-base reproduction:** Add this issue as a native blocker of #196 and proceed to Phase 1.
- **Non-deterministic after the exact matrix:** Retain `ready-for-agent` only while a concrete missing fixture is known. Otherwise document the matrix, apply `needs-info`, and request only the missing evidence.
...
- [ ] Prefer a test-only seam or a pure typed predicate result. Any retained production diagnostic must be privacy-safe, behavior-preserving, independently tested, and contain no identifiers or profile contents.
- [ ] Produce a red contract or browser regression that fails for the proven reason.
...
| --- | --- |
| Branch-only regression from #286 | Repair inside #286 under its existing `ready-for-agent` scope |
| Presentation says `Up to date` while the stricter safety state correctly refuses | Keep the fences; implement compatible status/action availability and persistent actionable feedback under `ready-for-agent` |
| Behavior-preserving defect in request propagation, stale handler state, or a predicate that demonstrably contradicts the already accepted owner/generation/revision state | Codex may update the issue with the proven narrow contract and implement under `ready-for-agent` |
| Any fix would relax ownership, lineage, generation, accepted-revision, pending, queued, dirty, connectivity, or activation safety | Apply `needs-triage`, present the exact tradeoff, and wait for product-owner direction |
| Database/RPC defect | Split a database ticket with explicit migration, RLS/grant, pgTAP, rollback, and deployed-schema requirements before implementation |
...
- [ ] Run `supabase/tests/learner_profile_start_over.test.sql` and prove CI invokes it only if RPC/database behavior changes.
...
- **Reproduced:** Preserve the smallest red fixture and exact source location. Create a narrow follow-up automatically as `ready-for-agent` only if the contract is behavior-preserving: retain the original Anki error, avoid the secondary null-target exception, and change no Activity-log policy. Otherwise apply `needs-triage` and state the unresolved behavior choice.
- **Not reproduced:** Record all five cases, Settings states, tested browser project, contract coverage, and history result; then close without speculative production changes.

### Boundary

Keep the investigation local. Do not enable or exercise Anki integration on the hosted site. Logging suppression, deduplication, or coalescing requires a separate product contract and ticket.

## #196 coordination

After tracker preparation is authorized and the stable sources exist:
...
3. Update #196 with stable evidence links, every issue link, and the exact dependency rules below.
...
1. Publish the reviewed plan and sanitized evidence.
2. Reconcile the exact five labels.
3. Update #286, create Packets 2–7, and update #196 using the rules above.

### Local issue stage

4. Complete #286 against the current base.
5. Run Packet 6 Phase 0 early so a branch-only regression is folded into #286 rather than becoming duplicate work.
6. Complete Packets 3, 4, and 5 independently once their current-base red reproductions exist.
7. Run the bounded Packet 7 investigation without delaying confirmed defects.
8. Run Packet 2's read-only provider decision stage; request the single trust-model choice only after Codex posts a recommendation and rollback.

### Delivery stage

9. Deliver each source issue through a narrow PR only when `delivery` authority names the permitted operations.
10. Execute approved provider configuration through an interactive Codex task; keep the external mutation and repository documentation diff independently reviewable.
11. Perform only ticket-specific non-destructive hosted smokes that receive `hosted-safe-smoke` authority.
...
The final gate may start only when all of these machine-checkable preconditions hold:

- [ ] #286 is closed after its deployed narrow smoke.
- [ ] Every source issue classified as a #196 blocker is closed against its final deployed change.
- [ ] Packet 6 has a recorded attribution outcome; if current-base reproduction succeeded, its resulting blocker is closed.
- [ ] Packet 3 has an explicit blocker/non-blocker decision mapped to a named #196 criterion.
- [ ] Packet 2's chosen provider state is applied and verified, or the issue records why no provider change is required.
- [ ] No release-relevant PR is open or waiting to merge.
- [ ] The intended release commit and runtime configuration are final and their exact identities are recorded.
...
- [ ] No unresolved P0/P1 correctness, ownership, integrity, or progress-loss finding remains.
...
- [Corrected issue-plan review](internal-canary-issue-plan-corrected-2026-09-04.md)
