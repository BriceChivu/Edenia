...
- [ ] Complete automated tests, realistic browser checks, security review, documentation, and a review-style cleanup pass.
...
- Observation: Browser base64url expands ciphertext by roughly one third. An 8 MiB plaintext ceiling does not fit inside an 8 MiB JSON request ceiling.
  Evidence: The server must distinguish plaintext, ciphertext, and encoded-request caps. Start with 8 MiB plaintext/ciphertext and an approximately 11 MiB bounded JSON body, then prove that exact size in the approved Supabase environment before launch or revise the limit without truncation.
...
In `supabase/functions/_shared/legacy-progress-transfer.ts`, export dependency-injected handlers or one action router whose external schemas are exact:
    ...
    -> { status: 'claimed', iv, ciphertext, expires_at }
