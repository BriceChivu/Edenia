...
- [ ] Publish the public-on verifier and final launch evidence, obtain exact-head CI, merge the focused follow-up, and reverify the deployed merge without changing the approved live controls.
...
Public-launch outcome as of 2026-08-14 07:25Z: accountless migration is live at `https://www.edenia.study/` with relay acceptance and consumption both enabled. A hosted no-state browser reaches the unchanged trailer/onboarding, while a hosted synthetic valid-state round trip proved encrypted create/claim/complete, canonical URL scrubbing, primary restoration, an IndexedDB recovery backup, migration activity, source byte/hash preservation, and no observed PostHog capability/state leak. The production kill-switch rehearsal proved that disabling acceptance plus the app flag while retaining consumption yields ordinary startup and zero migration calls; the approved enabled state was then restored in successful Pages run `31779768076`. Final relay state has zero noncompleted/ciphertext rows, active successful five-minute cleanup, and privacy-clean bounded Edge logs. The implementation outcome is complete subject only to publishing this final verifier/evidence follow-up; operational retention begins at `2026-08-14T07:24:22Z`, cannot be eligible before `2027-01-14`, and additionally requires a rolling 90-day zero-completion window plus separate retirement approval.
