...
- [ ] (2026-08-13 15:37Z) Blocked pending explicit approval for the GitHub CI-fix workflow's smallest remedy: conditionally start the fixed `localhost:8000` `_site` test server in `playwright.config.mjs` only when the ordinary configured port differs, then run the 11 focused cases, commit, push, and require exact-head CI. This preserves the broader suite's production-like `4173` origin and changes no product runtime or test assertion. The same approval blocker has repeated for three consecutive resumed goal turns; no further read-only evidence can reduce it.
...
Draft-review outcome as of 2026-08-13: PR #140 is open, draft, and mergeable, but its required `verify` check is red on a deterministic test-server topology mismatch. The failure does not contradict the migration logic: exact-head remote evidence passed the build, contract, shared backend, Deno, and isolated database layers, while a local CI-topology reproduction proved all 11 affected cases pass when their fixed test origin is actually served. Implementing and pushing the config-only repair awaits explicit approval under the CI-fix workflow.
