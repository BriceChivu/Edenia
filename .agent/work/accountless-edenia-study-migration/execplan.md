...
- [ ] The next implementation slice requires explicit owner approval for the named live operations: publish the separate helper repository, apply the disabled relay/corrective migrations and functions plus cleanup proof, then configure Pages/DNS/provider allowlists for a backed-up canary. None of those operations, the public enablement, or retirement is authorized by commit-push/conditional-merge permission.
