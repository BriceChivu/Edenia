...
- [x] (2026-08-13 16:32Z) Observed local, remote branch, and draft PR #140 at the published reconciliation head `4697bedafeade98ccad83af5d4099648ed1ae206`. Its exact-head run passed build, 1,057 contracts, 131 shared backend tests, every Deno check, all 606 database assertions, and the serial browser step with 215 passed and 503 intentionally skipped. PR #140 remained draft, mergeable, and clean. No provider or public-runtime state changed.
- [x] (2026-08-13 16:30Z) Rejected the proposed hosted history repair after a deeper read-only catalog comparison. The two unrecorded billing migrations were not fully applied: later recorded `ensure_*` migrations exactly own the live rate-limit, webhook, and cancellation prerequisites, while the live cloud-backup policies omit the seven-day `past_due` restriction and intentionally retain no UPDATE policy after backups became append-only. Removed the two obsolete unrecorded source files and added one new additive reconciliation that backfills missing `past_due_since`, enforces the seven-day SELECT/INSERT window, and explicitly preserves immutable browser backups. A fresh Supabase bootstrap applied only recorded history plus the relay and new reconciliation; 1,057 contracts, 620 assertions across fifteen database files, and local security/performance advisors passed. A linked dry run now cleanly proposes only `20260813132440_legacy_progress_transfer_relay.sql` and `20260813162549_reconcile_plus_backup_grace_policies.sql`; no migration-history mutation is needed.
- [x] (2026-08-13 16:34Z) Committed the corrected source slice locally and completed a fresh commit-scoped review. The review found one bounded mismatch between the migration's fail-closed comment and its grant revocation; the SQL and test now revoke/assert no UPDATE for `PUBLIC`, `anon`, and `authenticated`. A second fresh bootstrap, six focused source contracts, 14 behavioral policy assertions, and both local advisor classes passed.
- [x] (2026-08-13 16:55Z) Owner authorized pushing commits and merging PR #140 if necessary. Pushed focused correction commit `049cb210143054b376675b455ba6c2b4c41335ed`; exact-head CI run `31721907006` passed in 13m01s with the build, 1,057 contracts, 131 shared backend tests, every Deno check, 620 database assertions across fifteen SQL suites, and 215 browser passes with 503 intentional skips. The remote head matched the local commit and PR #140 returned to mergeable/clean. Merge remains deliberately deferred because it is neither necessary nor safe before the helper, disabled hosted relay, Pages/DNS, and provider sequence exists.
- [x] (2026-08-14 02:48Z) Owner resumed the specifically named staged live operations: publish and verify the separate helper; apply the relay and corrective migration, functions, and cleanup while controls remain disabled; then prepare Pages/DNS/provider allowlists and a backed-up private canary. Public migration enablement and retirement remain separate future approvals.
- [x] (2026-08-14 03:04Z) Reverified the merged implementation and every first-write target. Owner-merged PR #140 produced master and Pages SHA `a33d99cf2c5f12f695d27fd14cb29570f28f0a0d`; the old Pages URL returned `200` with no public migration variable. The linked `Edenia Plus` project was healthy; only the relay and grace-policy migrations were pending; both relay controls, relay rows, and relay metrics started false/zero; the browser roles had no relay table/RPC access; and the affected backup policies had no pending data backfill. Current official Supabase function, API-key, Cron, limits, and breaking-change guidance plus CLI 2.111.0 were rechecked before writes.
- [x] (2026-08-14 03:04Z) Applied only migrations `20260813132440` and `20260813162549` to `Edenia Plus`, retaining `acceptance_enabled=false` and `consumption_enabled=false`, zero relay rows/metrics, private RLS, service-only RPC grants, and append-only seven-day backup policies. Deployed both named relay functions with custom named publishable-key authentication. The first live browser probe found allowed CORS preflight blocked by the wrapper at `401`; a narrow source fix now validates only `OPTIONS` before authentication, while every data-bearing request still traverses `publishable:default`. Focused tests, 61 database assertions, local lint, and full `npm test` passed with 1,058 contracts and 131 shared function tests; the corrected live probe returned allowed `204`, forged-origin `403`, missing/malformed-key `401`, and disabled create/consume `503`.
...
- [x] (2026-08-14 07:25Z) Launched the accountless migration in dependency order and proved the hosted outcome. Relay consumption became true before acceptance, the repository flag then deployed true from master SHA `3adbfd0b56206cca686e313fd5a16c576f40a30f`, and live no-state plus synthetic valid-state browser checks passed. The valid transfer restored one channel, one video, and one Anki day; recorded completion; stored the protected `legacy origin recovery` copy in production's IndexedDB backup store; scrubbed the fragment; left no capability/ciphertext in destination storage or observed PostHog traffic; and left the old-origin JSON byte- and hash-identical. The public switch-off rehearsal then set acceptance false, kept consumption true, deployed the application flag false in run `31779646353`, and proved ordinary onboarding with no helper navigation, migration marker, or relay call and no in-flight/ciphertext row. Acceptance and the application flag were restored only after that passed. Final run `31779768076` succeeded at 07:24:22Z with controls true/true, zero noncompleted or ciphertext-bearing rows, and five anonymous completions total. The five-minute cleanup was active and its 07:25Z run succeeded; bounded Edge logs contained no sensitive-field or 5xx match. The explicitly public-on domain verifier passed 11/11, and a final fresh no-state browser again returned from one helper check to normal onboarding without a relay write.
- [x] (2026-08-14 07:25Z) Converted retention into an explicit operational follow-up rather than leaving implementation perpetually active. The final public launch timestamp is `2026-08-14T07:24:22Z`; the earliest five-calendar-month eligibility date is `2027-01-14`. Helper/relay retirement also requires the preceding 90 days of anonymous daily completion metrics to remain zero, and any later completion resets that quiet window. Retirement, deletion of the old helper, and disabling relay consumption remain separate future approvals. Real Safari on macOS/iOS, Firefox, Edge, and restrictive/private-mode coverage remains unverified and must not be inferred from the green Chromium/WebKit automation and real Chrome canary.
- [x] (2026-08-14 07:42Z) Published the public-on verifier and final launch evidence in PR #145 at exact head `fb9db1d631f36526962ff4ede26eed465c898453`. CI run `31780145391` passed the build, complete contract stage, and serial browser gate in 10m25s; unrelated Supabase/Deno/database stages were correctly skipped by file scope. The clean PR merged as `5eaca7085ccb96d32cdfcb1eb09d695c92f3960d`, and Pages run `31780871151` deployed that exact merge successfully. The post-merge audit passed all 11 live domain checks with migration=true, checkout=false, and accounts=internal; Pages remained built with the verified `www` domain and enforced HTTPS; relay controls remained true/true with zero noncompleted/ciphertext rows; cleanup remained active/succeeded; and bounded Edge logs remained free of sensitive-field and 5xx matches. The implementation work item is complete; only the explicitly recorded retention monitoring and separately approved future retirement remain.
...
- Observation: The hosted migration history had twelve reminder/account versions that were absent from source, while source had the same twelve migration names under different timestamps.
  Evidence: A read-only query proved every remote statement set matched its same-named source SQL after comment/whitespace normalization. Renaming source to the deployed identities removed that mismatch.

- Observation: Object existence was too weak to justify marking the two unrecorded billing migrations applied.
  Evidence: Recorded migrations `20260812000351_ensure_account_export_rate_limit.sql` and `20260812002050_ensure_stripe_webhook_prerequisites.sql` own the live tables, RPCs, grants, and cancellation column. A deeper live-policy query showed that `20260801042407_harden_stripe_billing_lifecycle.sql` was not fully applied: SELECT/INSERT still allowed every `past_due` subscription and the UPDATE policy remained absent after backups became append-only. Deleting the obsolete unrecorded source files and adding one current corrective migration made linked dry-run history unambiguous without falsifying metadata.
...
- Observation: Applying `withSupabase({ auth: 'publishable:default' })` to an entire Edge Function also authenticates browser `OPTIONS` preflights, which carry no API key and therefore fail before strict CORS can answer them.
  Evidence: The first hosted allowed-origin probe returned `401`. The corrected entrypoints route only `OPTIONS` directly to the shared exact-origin preflight response; every data-bearing request still traverses publishable-key authentication. Hosted probes now return allowed `204`, forged-origin `403`, missing/malformed-key `401`, and disabled create/consume `503`.
...
- Observation: Production backup cleanup can leave `edenia_v1_backups` empty even after the migration has created and verified the required recovery copy.
  Evidence: The first hosted synthetic assertion looked only at localStorage and reported zero backups after an otherwise complete transfer. Reading `edenia_state_backups_v1/backups` in the same destination profile found the exact `legacy origin recovery` entry and source marker, with the IndexedDB activation marker set. Live release checks must inspect the configured backup repository rather than assume its legacy mirror remains populated.

- Observation: The original domain verifier deliberately hard-coded the prelaunch migration-off state, so it became an expected failure after approved public enablement.
  Evidence: The first final audit passed ten deterministic checks and failed only `automatic legacy migration is not false`. The verifier now defaults to expected-off but accepts an explicit strict `EDENIA_EXPECT_LEGACY_PROGRESS_MIGRATION_ENABLED=true` for public-on audits; six focused contracts prove both states and reject ambiguous values.
...
- Decision: Reconcile source migration filenames to exact hosted versions only when the hosted statement set matches the same-named source SQL, and freeze those identities and bytes in the repository contract.
  Rationale: This repairs deployment history without re-running already-applied SQL or deleting hosted migration records. The two billing history gaps were not safe to infer away, and deeper evidence confirmed that no hosted history repair should be made.
  Date/Author: 2026-08-13 / Codex

- Decision: Remove the two obsolete unrecorded billing migrations and express only the missing Plus-backup grace rule as a new additive migration that preserves append-only browser access.
  Rationale: Later recorded `ensure_*` migrations already provide the live service-owned billing primitives and cancellation column. Marking the old files applied would falsely claim that their policy changes ran; applying them would fail on existing tables and reintroduce UPDATE access that the backup-history migration deliberately removed. A current corrective migration has one clear job, matches the application's seven-day entitlement rule, and can be reviewed and deployed normally.
  Date/Author: 2026-08-13 / Codex
...
- Decision: Do not attach the Pages custom domain until the exact DNS replacement can immediately follow, and keep every migration control off through DNS/TLS smoke testing.
...
- Decision: Keep the live-domain verifier's default expectation at migration off and require operators to state an expected public-on rollout explicitly.
  Rationale: A verifier that accepts either state would not catch an accidental enable or disable. An explicit boolean preserves the fail-safe default while letting the same deterministic audit validate the owner-approved live state.
  Date/Author: 2026-08-14 / Codex

- Decision: Start the five-calendar-month retention clock from the final restored public deployment, not the earlier temporary enable that preceded the kill-switch rehearsal.
  Rationale: `2026-08-14T07:24:22Z` is the first point after which the approved final configuration remained live. The old helper and relay must remain through at least `2027-01-14` and until a rolling 90-day zero-completion window is observable; retirement is not part of this implementation completion.
  Date/Author: 2026-08-14 / user and Codex
...
Public-launch outcome as of 2026-08-14 07:42Z: accountless migration is live at `https://www.edenia.study/` with relay acceptance and consumption both enabled. A hosted no-state browser reaches the unchanged trailer/onboarding, while a hosted synthetic valid-state round trip proved encrypted create/claim/complete, canonical URL scrubbing, primary restoration, an IndexedDB recovery backup, migration activity, source byte/hash preservation, and no observed PostHog capability/state leak. The production kill-switch rehearsal proved that disabling acceptance plus the app flag while retaining consumption yields ordinary startup and zero migration calls; the approved enabled state was then restored and retained through PR #145's exact-head CI, merge `5eaca7085ccb96d32cdfcb1eb09d695c92f3960d`, and successful Pages deployment `31780871151`. The post-merge 11/11 live verifier, relay, cleanup, and log-privacy audit remained green with zero noncompleted/ciphertext rows. The implementation outcome is complete. Operational retention begins at `2026-08-14T07:24:22Z`, cannot be eligible before `2027-01-14`, and additionally requires a rolling 90-day zero-completion window plus separate retirement approval.
...
Read-only launch audit refreshed at 2026-08-13 16:16Z:
    ...
    Source/hosted migration history: twelve exact SQL identities reconciled and published;
      deeper live evidence forbids marking two older billing files applied; local source now
      removes those obsolete files and proposes one new append-only grace-policy correction
...
Staged-live audit at 2026-08-14 03:26Z:

    Edenia source/Pages SHA: ca43664be2800c705612575b58bb5a30f1f0469e
    Edenia Pages deployment: run 31766478658, success; cname=null; old URL HTTPS 200
    Helper source SHA: 0f98a54efe598bbe8111ede36cdff148710de230
    Helper Pages deployment: build 1150233811; HTTPS 200; four reviewed hashes exact
    Supabase migrations: relay, Plus backup grace policy, and tracked cleanup schedule applied
    Relay functions: corrected strict preflight deployed; data requests retain publishable-key auth
    Relay controls/data: acceptance=false; consumption=false; rows=0; metrics=0
    Cleanup: edenia-legacy-progress-transfer-cleanup active every five minutes;
      latest run at 2026-08-14 03:25Z succeeded with DELETE 0
    Hosted grants: no browser table access; no browser RPC execute; service-only RPC execute
    Hosted advisors: intentional private deny-all RLS notices plus pre-existing non-relay notices;
      no browser-reachable relay-policy or privilege finding
    Supabase Auth: canonical www Site URL; exact transition/local callbacks retained; no wildcard
    Supabase server return roots: canonical www; checkout and reminder delivery remain disabled
    Google: exact www YouTube referrers and OAuth JavaScript origin added; old/local retained
    PostHog: no visible authorized-domain setting; source starts analytics only at canonical www;
      helper includes no analytics
    Stripe: server-side subscription Checkout and Customer Portal paths derive canonical APP_URL;
      webhook remains at Supabase; no client-side domain callback exists; checkout remains disabled
    GitHub domain verification: exact ownership challenge prepared but TXT not yet published
    DNS: apex 192.64.119.36; www parkingpage.namecheap.com; no cutover write performed
...
Domain-cutover and private-canary audit at 2026-08-14 04:31Z:

    Current master/Pages SHA: 3adbfd0b56206cca686e313fd5a16c576f40a30f
    Current Pages deployment: run 31770041791, success
    Post-canary master delta: PR #144 changed only data/channel-catalog.candidates.json;
      migration, helper, relay, fragment escrow, and verifier surfaces unchanged
    GitHub domain: edenia.study ownership verified; Pages cname=www.edenia.study;
      protected domain verified; HTTPS enforced; status built
    DNS: exact four GitHub Pages apex A records; no apex AAAA; www CNAME exact;
      ownership TXT retained; existing DMARC, DKIM, SPF, and mail MX retained
    Live domain verifier: 11/11 passed after local parking-cache expiry
    Public runtime: migration=false; checkout=false; accounts=internal
    Pre-canary backup: manual normal sync export copied out of band; byte and SHA exact
    No-state evidence: isolated canonical conclusive-none return opened current trailer/onboarding;
      exact helper/app round-trip suites passed 13/13 desktop and 13/13 WebKit
    Real Chrome valid-state evidence: same 788 points, four-day streak, 1,592 videos,
      31 Anki days, one source-hash migration marker, and visible recovery backup
    Old-origin preservation: post-canary export state JSON byte-identical to pre-canary export
    Relay final state: acceptance=false; consumption=false; completed rows=1;
      completed metric=1; ciphertext-bearing rows=0; cleanup active
    Hosted relay log scan: no ciphertext/capability field, bearer value, or PostHog match
    Focused contracts: 15/15 passed; focused desktop browser: 13/13 passed;
...
Plan revision 2026-08-13 (billing-history contradiction): Replaced the unsafe assumption that two object-present billing migrations could be marked applied with live catalog proof of partial/non-equivalent policy state. Removed the obsolete unrecorded files, introduced one additive seven-day grace migration that preserves append-only backups, added behavioral database coverage and CI routing, and proved a clean two-migration linked dry run without mutating hosted history.

Plan revision 2026-08-13 (fresh review of billing-history correction): Recorded green exact-head CI for the published reconciliation and tightened the local corrective migration so UPDATE is revoked from every browser-reachable role, matching its fail-closed append-only contract. Repeated fresh bootstrap, focused contracts, policy behavior, and advisors after the review fix.

Plan revision 2026-08-13 (follow-up publication authorization): Recorded the owner's broader commit-push and conditional merge authorization, the clean pre-push review, and the decision to keep the draft unmerged until coordinated deployment is actually safe. Live Supabase, helper, Pages, DNS, provider, canary, public-enable, and retirement actions remain separately approval-gated.

Plan revision 2026-08-13 (green billing-policy follow-up): Recorded the exact published source SHA, complete exact-head CI evidence, clean PR state, and the release-sequencing reason not to merge yet. The work item returns to the explicit live-operation approval boundary rather than treating GitHub publication as production completion.

Plan revision 2026-08-14 (staged live-operation approval): Recorded the owner's instruction to continue with the previously named helper, disabled relay, domain/provider preparation, and private-canary sequence. The public migration flag remains outside this approval so every live write must preserve accountless public behavior until a later evidence review.
...
Plan revision 2026-08-14 (public launch and switch-off rehearsal): Recorded the consumption-before-acceptance public enable, hosted no-state and valid-state evidence, production IndexedDB recovery-copy location, fragment/PostHog/source-preservation checks, successful acceptance-off/application-off drain rehearsal, final restored Pages deployment and relay state, corrected expected-rollout verifier, active cleanup and log-privacy evidence, and the `2027-01-14` earliest retention eligibility date. Implementation can complete after the focused evidence follow-up is published and merged; the rolling 90-day quiet window and any retirement remain operationally tracked, separately approved work.

Plan revision 2026-08-14 (implementation closeout): Recorded PR #145's exact-head green CI, merge SHA, exact deployed Pages run, and post-merge public-on/domain/relay/cleanup/privacy audit. The implementation lifecycle is now complete; the old helper and relay remain intentionally live for the five-month minimum and rolling 90-day quiet-window operations, and retirement remains a separate future approval.
