...
- [x] (2026-08-15 01:34Z) Completed repair PR #150. Final exact head `71e461b` passed the strict required `verify` in run 31856369518, the PR became ready only after that result, and protected merge produced `18dd46f` without administrator bypass. Pages run 31856872515 then deployed that exact merge successfully.
- [x] (2026-08-15 01:44Z) Repeated the protected email-to-PostHog canary on the exact repair deployment. One approved magic link was transferred directly from the provider preview to the confirming browser without rendering its address or capability in output. The fragment was scrubbed before interaction, deliberate confirmation signed in only that browser, the requesting browser remained signed out with one Turnstile widget, and the PostHog search returned one internal person whose `auth_method` is now `email`, not Google. The confirming browser was explicitly signed out and remained signed out after reload.
- [x] (2026-08-15 01:44Z) Reverified the live safety state after repair: a cache-busted runtime fetch reports account rollout `internal`, Google `id_token`, One Tap enabled, and both public provider identifiers present; the ordinary public root renders no Account state and loads neither GIS nor Turnstile; the Pages source SHA is exact. All provider plans remain Free and reminder delivery remains independently off.
- [x] (resolved 2026-08-15 02:35Z) Diagnosed the hosted GIS `script` failure that blocked the One Tap evidence: the forced anonymous-CORS script request failed before Google initialized. PR #152 exact head `f77e2e9` passed required `verify` run 31859092136, merged as `3c1a443`, and deployed successfully in Pages run 31859597726. The standard script tag now renders Google's official button in both Chrome and Safari, and `itp_support` is enabled for the supported Safari/Firefox prompt UX.
...
- [x] (2026-08-15 02:21Z) Used the user's manual Chrome and Safari evidence to reproduce the missing official button and absent prompt. The loader incorrectly set `crossorigin="anonymous"` on Google's classic script even though the response has no `Access-Control-Allow-Origin`; a differential real-browser harness proved the old tag fails and the standard tag loads `window.google.accounts.id` in both Chromium and WebKit. Removed the attribute, enabled Google's `itp_support` option for Safari/Firefox, added exact initializer/loader regressions, and passed nine controller contracts plus 14 affected desktop/phone browser cases with 40 intentional project skips.
- [x] (2026-08-15 02:57Z) Reproduced the next hosted manual-button failure against Supabase Auth logs without retaining credential data. Google returned the ID token, but every exchange failed with `invalid nonce: Nonces mismatch`: Edenia encoded the SHA-256 digest as Base64URL while Supabase's current Google web contract expects lowercase hexadecimal. Added an exact raw-to-hashed regression, changed only the digest representation, and passed 30 focused auth contracts, full `npm test` with 1,099 contracts plus 131 shared Supabase tests and all Deno checks, and the focused browser suite.
- [x] (2026-08-15 02:57Z) Completed nonce repair PR #153. Exact head `098662e` passed required `verify` run 31860058903 in 9m19s, merged through protection as `8abccf4`, and deployed successfully in exact Pages run 31860487920. A cache-busted hosted Chrome canary rendered the official button, opened Google's identity chooser, completed in Edenia without a top-level Supabase navigation or generic error, and produced a clean server-side `id_token` exchange with status 200. Explicit sign-out then kept a fresh eligible page signed out, preserving immediate-reauth suppression.
- [x] (2026-08-15 03:18Z) Completed the final hosted One Tap acceptance on exact deployment `8abccf4`. A fresh eligible Chrome lifecycle displayed Google's browser-owned identity prompt only after onboarding and walkthrough resolution; closing the prompt with its native dismiss control left Edenia usable and signed out. A separate eligible Chromium profile then moved from signed out with GIS ready to signed in with GIS complete without clicking the manual button, and Supabase Auth recorded the matching status-200 `id_token` exchange. Its 40-card local feed remained present through automatic sign-in and explicit sign-out. Temporary captures were deleted immediately and retained no account address or authentication capability. Only protected delivery and exact Pages verification of this evidence closeout PR remained at branch time.
- [x] (2026-08-15 03:39Z) Completed evidence closeout PR #151. Final exact head `d643992` passed required `verify` run 31861852918 in 10m59s after rebasing onto the newly merged community-catalog change, then merged through protection as `600121b`. Pages run 31862379844 deployed that exact merge successfully. The completed work item is now recorded in this final metadata-only closeout.
...
- Observation: A successful HTTP 200 was insufficient because Edenia set `crossorigin="anonymous"` on the GIS classic script. Google's response permits cross-origin embedding but does not send `Access-Control-Allow-Origin`, so Chromium and WebKit rejected the forced CORS fetch and never installed `window.google.accounts.id`; the empty official-button mount and missing prompt were the same failure. Safari also requires `itp_support: true` for Google's upgraded One Tap UX, whose documented limitation is no automatic sign-in.
  Evidence: manual Chrome and Safari screenshots after onboarding/walkthrough completion; live `failureStage=script`; response-header inspection; and a differential Playwright harness where the crossorigin tag failed in Chromium/WebKit while the standard tag loaded successfully in both.

- Observation: FedCM/One Tap is browser-owned UI. A hosted prompt can be visibly present while Edenia's DOM and page-scoped screenshot contain no prompt element; `googleIdentityStatus=ready` proves provider readiness but not prompt visibility. The final canary therefore required direct browser-window observation for visibility and the prompt's own close control for dismissal.
  Evidence: the eligible hosted Chrome page on exact deployment `8abccf4` displayed Google's identity prompt above an otherwise prompt-free page DOM, and native dismissal removed it while Edenia remained usable and signed out.

- Observation: A Chrome profile with multiple active Google sessions correctly presents an identity chooser and cannot prove automatic selection. A separate eligible single-session Chromium profile completed sign-in without any manual Google-button click, while the multi-session Chrome profile supplied the independent real-prompt and dismissal proof.
  Evidence: signed-out-to-signed-in GIS state transition, matching status-200 Supabase `id_token` exchange at 2026-08-15 03:18Z, and the unchanged 40-card local feed through automatic sign-in and explicit sign-out.

- Observation: The first real credential exchange exposed a second independent defect after the script repair. Google correctly returned an ID token containing Edenia's supplied digest, but Supabase hashes the raw nonce and compares its lowercase hexadecimal SHA-256 representation. Edenia supplied the same digest bytes in Base64URL form, so the values were cryptographically equivalent but textually unequal.
  Evidence: repeated hosted Auth `id_token` failures with the single safe error `invalid nonce: Nonces mismatch`; Supabase's current Google pre-built sign-in documentation; the exact raw-to-hex contract; and the first status-200 hosted `id_token` exchange after PR #153.
...
- Observation: The current-method repair works on the exact hosted merge. A fresh magic-link session on the Google-owned UUID updates the single searchable PostHog person to `auth_method=email`; provider metadata no longer overrides the verified session method.
  Evidence: PR #150, exact Pages run 31856872515, direct one-use link transfer, isolated cross-device confirmation, and the authenticated PostHog person-property audit on 2026-08-15.

- Observation: The hosted One Tap gap is external to Edenia's now-instrumented initialization path. After the repair deployment, a fresh eligible signed-out requester again reported `googleIdentityStatus=unavailable` and `googleIdentityFailureStage=script` before the GIS global existed. The page remained usable and signed out.
  Evidence: exact deployment `18dd46f`, fresh internal browser lifecycle with no onboarding or walkthrough visible, and absence of both the GIS global and credential-picker container on 2026-08-15.
...
- Decision: Load the GIS classic script exactly as a standard cross-origin script without a `crossorigin` attribute, and initialize every opportunity with `itp_support: true`.
  Rationale: Google's script response is not CORS-enabled, while its upgraded Safari/Firefox One Tap UX is opt-in and defaults off. This restores the official button in normal browsers and enables the supported ITP prompt without pretending Safari can prove automatic sign-in.
  Date/Author: 2026-08-15 / Codex after manual Chrome and Safari canary evidence.

- Decision: Keep the raw nonce as a random Base64URL string, but pass Google the SHA-256 digest as lowercase hexadecimal and pass Supabase the original raw value.
  Rationale: This is Supabase's documented Google web contract. It preserves the one-use in-memory nonce lifecycle while making the provider and verifier compare the same representation.
  Date/Author: 2026-08-15 / Codex after hosted nonce-mismatch evidence.
...
Implementation PR #148, repair PRs #149, #150, #152, and #153, and evidence PR #151 were protected-merged and exactly deployed. Dedicated Resend SMTP, the exact hosted branded template, Turnstile, protected cross-device email confirmation, manual and automatic Google ID-token sign-in, a visible eligible One Tap prompt and native dismissal, same-email UUID convergence, separate local-state preservation, session restoration, explicit-sign-out suppression, phone geometry, PostHog searchability, verified current-method updates in both directions, and the complete ordered rollback rehearsal have live evidence. The ordinary public path remains accountless and provider-script-free, all selected services remain Free, and reminder delivery is unchanged. Google's official button renders in Chrome and Safari, and both manual and automatic hosted ID-token exchanges succeed server-side without top-level Supabase navigation. All product and release acceptance criteria are complete.
...
Plan revision 2026-08-15 (repair deployment and evidence): Recorded PR #150 exact-head CI, protected merge, exact Pages deployment, and the successful email-to-PostHog repair canary. Reconfirmed the internal runtime and ordinary public path. Kept the work item active because a visible hosted One Tap prompt still requires a real non-automated browser; controlled browser evidence continues to stop safely at the external GIS script boundary.

Plan revision 2026-08-15 (GIS and nonce repairs): Recorded PR #152's standard script loading and ITP support, PR #153's hexadecimal nonce correction, both exact-head protected merges and Pages deployments, and the successful hosted manual Google ID-token exchange. Retained only the independent hosted One Tap prompt, dismissal, and Chromium automatic-sign-in proof as incomplete.

Plan revision 2026-08-15 (final hosted prompt evidence): Recorded the real Chrome browser-owned One Tap prompt and native dismissal, plus automatic sign-in in a separate eligible Chromium profile without a manual-button click. The matching successful Auth exchange and unchanged local study feed complete hosted product acceptance; only the evidence PR's protected merge and exact deployment verification remained at branch time.

Plan revision 2026-08-15 (completion): Recorded evidence PR #151's rebased exact head, protected `verify`, merge commit, and exact Pages deployment. With every Progress item complete and `meta.json` transitioned to `implementation/completed`, the internal-canary work item is closed.
