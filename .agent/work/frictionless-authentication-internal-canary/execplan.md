...
- [ ] Obtain one visible hosted eligible One Tap prompt, exercise dismissal, and prove Chromium automatic sign-in in a fresh eligible browser state; then finish this evidence closeout PR through protected merge and exact deployment proof. Do not use Safari as automatic-sign-in evidence because Google's ITP UX does not support that mode, and do not defeat Edenia's explicit-sign-out suppression to manufacture a result.
...
Implementation PR #148 and repair PRs #149, #150, #152, and #153 were protected-merged and exactly deployed. Dedicated Resend SMTP, the exact hosted branded template, Turnstile, protected cross-device email confirmation, manual Google ID-token sign-in, same-email UUID convergence, separate local-state preservation, session restoration, explicit-sign-out suppression, phone geometry, PostHog searchability, verified current-method updates in both directions, and the complete ordered rollback rehearsal now have live evidence. The ordinary public path remains accountless and provider-script-free, all selected services remain Free, and reminder delivery is unchanged. Google's official button now renders in Chrome and Safari, and the corrected hosted ID-token exchange succeeds server-side without top-level Supabase navigation. The only incomplete product acceptance is a visible hosted eligible One Tap prompt plus dismissal and Chromium automatic-sign-in proof in a fresh eligible browser state; local desktop/phone tests cover those policies but cannot replace the hosted observation. The evidence closeout PR remains draft until that proof exists. At final completion, summarize the manual and automatic Google sign-in behavior, email delivery and confirmation behavior, PostHog identity result, same-email UUID proof, local-state preservation evidence, external configuration, all PRs and exact merged/deployed SHAs, switch-off rehearsal, and any browser/provider limitations discovered.
