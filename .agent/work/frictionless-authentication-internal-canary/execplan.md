...
- [ ] (blocked evidence 2026-08-15 01:44Z) Obtain one visible hosted One Tap/automatic-sign-in prompt in a real non-automated browser, exercise dismissal and automatic sign-in, then finish the evidence closeout PR through protected merge and exact deployment proof. The fresh eligible requester on exact deployment `18dd46f` again reached safe failure stage `script` before `window.google.accounts.id` existed. Local desktop/phone tests prove prompt gating, cancellation, and explicit-sign-out suppression, but cannot substitute for this hosted proof.
...
- [ ] Deploy the GIS script-request and Safari ITP repair; repeat the hosted manual-button, eligible prompt, dismissal, and Chromium automatic-sign-in canaries; then deliver the evidence closeout PR through protected merge and exact deployment proof.
...
Implementation PR #148, repair/provider-stage PR #149, and current-method repair PR #150 were protected-merged and exactly deployed. Dedicated Resend SMTP, the exact hosted branded template, Turnstile, protected cross-device email confirmation, manual Google ID-token sign-in, same-email UUID convergence, separate local-state preservation, session restoration, explicit sign-out, phone geometry, PostHog searchability, verified current-method updates in both directions, and the complete ordered rollback rehearsal now have live evidence. The ordinary public path remains accountless and provider-script-free, all selected services remain Free, and reminder delivery is unchanged. The only incomplete product acceptance is a visible hosted One Tap/automatic-sign-in prompt in a real non-automated browser: every available controlled hosted surface fails safely at the external GIS script boundary before initialization, while local desktop/phone browser tests cover eligibility, cancellation, automatic sign-in, and sign-out suppression. The evidence closeout PR must remain draft and unmerged until that independent hosted proof exists. At final completion, summarize the manual and automatic Google sign-in behavior, email delivery and confirmation behavior, PostHog identity result, same-email UUID proof, local-state preservation evidence, external configuration, all PRs and exact merged/deployed SHAs, switch-off rehearsal, and any browser/provider limitations discovered.
