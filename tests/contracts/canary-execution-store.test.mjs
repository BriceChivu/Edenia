...
test('reopened repair preserves every closed cycle and fences the same parent again', t => {
  const { first, second, file } = fixture(t)
  first.acquire('runner-a', 1000, 1000)
  first.reconcileExpired(2001, { previousExecutorStopped: true, candidate, gate: 'off', evidenceHash })
  first.acquire('runner-b', 2002, 10000)
  first.suspendForRepair('runner-b', 2003, { issue: 300, evidenceHash })
  first.resumeAfterRepair('runner-b', 2004, { issue: 300, closureEvidenceHash: evidenceHash, candidate, gate: 'off' })
  // Simulate an existing store from before repair history was introduced.
  first.db.exec('DROP TABLE IF EXISTS repair_history')
  const reopened = new CanaryExecutionStore(file)
  t.after(() => reopened.close())
  const original = { ...first.checkpoint().repairs[0] }
  const freshEvidence = 'c'.repeat(64)
  reopened.reopenRepair('runner-b', 2005, { issue: 300, evidenceHash: freshEvidence })
  assert.equal(second.state().phase, 'repairing-derived')
  assert.deepEqual({ ...second.checkpoint().repairHistory[0] }, { sequence: 1, ...original })
  assert.equal(second.checkpoint().repairs[0].suspended_evidence, freshEvidence)
  assert.equal(second.checkpoint().repairs[0].closure_evidence, null)
  const suspended = second.checkpoint()
  assert.throws(() => reopened.reopenRepair('runner-b', 2006, { issue: 300, evidenceHash }), /settled safe/)
  assert.deepEqual(second.checkpoint(), suspended)
  assert.throws(() => second.beginOperation('runner-b', 2007, { ...intent, gate: 'off' }), /suspended parent/)
  second.resumeAfterRepair('runner-b', 2008, { issue: 300, closureEvidenceHash: freshEvidence, candidate, gate: 'off' })
  second.reopenRepair('runner-b', 2009, { issue: 300, evidenceHash })
  assert.equal(first.checkpoint().repairHistory.length, 2)
  assert.equal(first.checkpoint().repairHistory[0].closure_evidence, evidenceHash)
  assert.equal(first.checkpoint().repairHistory[1].closure_evidence, freshEvidence)
})

test('unsafe repair reopen attempts preserve both current state and history', t => {
  const { first } = fixture(t)
  const attempt = (owner, now, issue, pattern) => {
    const before = first.checkpoint()
    assert.throws(() => first.reopenRepair(owner, now, { issue, evidenceHash }), pattern)
    assert.deepEqual(first.checkpoint(), before)
  }
  first.acquire('runner-a', 1000, 1000)
  attempt('runner-a', 1001, 300, /settled safe/)
  attempt('runner-a', 2001, 300, /not current/)
  first.reconcileExpired(2001, { previousExecutorStopped: true, candidate, gate: 'off', evidenceHash })
  first.acquire('runner-b', 2002, 10000)
  attempt('runner-b', 2003, 300, /closed repair/)
  first.suspendForRepair('runner-b', 2004, { issue: 300, evidenceHash })
  first.resumeAfterRepair('runner-b', 2005, { issue: 300, closureEvidenceHash: evidenceHash, candidate, gate: 'off' })
  first.beginOperation('runner-b', 2006, { ...intent, gate: 'off' })
  attempt('runner-b', 2007, 300, /settled safe/)
  first.finishOperation('runner-b', 2008, { id: intent.id, outcome: 'completed', evidenceHash })
  first.db.prepare("UPDATE repairs SET closure_evidence = NULL WHERE issue = 300").run()
  attempt('runner-b', 2009, 300, /closed repair/)
  first.db.prepare('UPDATE repairs SET closure_evidence = ? WHERE issue = 300').run(evidenceHash)
  first.db.prepare("UPDATE execution SET phase = 'closed'").run()
  attempt('runner-b', 2010, 300, /settled safe/)
  first.db.prepare("UPDATE execution SET phase = 'preflight'").run()
  first.claimWatchdog('watch-reopen', 'runner-b', 2011, 5000)
  attempt('runner-b', 2012, 300, /containment reconciliation/)
})
