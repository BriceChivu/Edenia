...
test('test build contains empty public keys and safe release defaults', async () => {
  ...
  assert.match(source, /"learnerProfileLifecycleEnabled": false/)
...
test('Pages deployment retires permanent feature inputs and forwards remaining controls', async () => {
  ...
  assert.match(
    runtimeConfigWriter,
    /learnerProfileLifecycleEnabled: parseRuntimeConfigFlag\(\s*process\.env\.EDENIA_LEARNER_PROFILE_LIFECYCLE_ENABLED,\s*'EDENIA_LEARNER_PROFILE_LIFECYCLE_ENABLED'\s*\)/
  )
  assert.match(
    workflow,
