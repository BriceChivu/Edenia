...
test('the live worker is bounded and fences immediately before provider I/O', () => {
  ...
  assert.match(live, /p_delivery_mode: 'live'/)
  const tokenIndex = live.indexOf("client.rpc('store_reminder_unsubscribe_token'")
  const beginIndex = live.indexOf("client.rpc('begin_reminder_provider_attempt'")
  ...
  assert.match(live, /complete_reminder_without_send/)
